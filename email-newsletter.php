<?
	...
	if ($_POST['newsletter_submitted']) {
		...
		else if (!checkEmail($_POST['email_address'])) $errors[] = 'Please enter a valid email address';
		if (empty($_POST['postalcode'])) $errors[] = 'Please enter your zip code';
		else if (!eregi("^[0-9]{5}(-[0-9]{4})?$", $_POST['postalcode'])) $errors[] = 'Please enter a valid zipcode';

		if (!count($errors)) {
			$message = "Email Newsletter Subscription\n\n";
			...
			$replyTo = $_POST['email_address'];
			$subject = 'Email Newsletter Subscription';
			
			$to = 'newsletter@hiltonwoodbridge.com';
	...
	if ($_POST['unsubscribe_submitted']) {
		...
		if (empty($_POST['email_address'])) $errors = 'Please enter your email address';
		if (empty($errors)) {
			$message = "Email Newsletter Unsubscribe Request\n\n";
			$message .= "Email: ".$_POST['email_address']."\n\n";
			$message .= "Sent: ".date('l dS \of F Y h:i:s A')."\n\n";
			
			$from = $_POST['email_address'];
			$fromName = $_POST['email_address'];
			$replyTo = $_POST['email_address'];
			$subject = 'Email Newsletter Unsubscribe Request';
			$to = 'newsletter@hiltonwoodbridge.com';
			...
			$mail->ClearAllRecipients();		
			$unsubscribed = true;
			//echo 'mailing unsubscribe';
		}
		else {
			//echo 'not mailing unsubscribe';
		}
	}
	if ($valid) 
		$title = 'Thank you for requesting our newsletter.';
	else if ($unsubscribed)
		$title = 'You have unsubscribed from our newsletter.';
	else
              ...
              <td colspan="3"><table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr>
				  ...
				  <?
					if ($valid) {
				?>	<div class="copy">
				Look for Hilton Woodbridge’s latest news and events coming soon to your inbox.
				<br/><br/>Redesigned. Reconsidered. Rediscover Hilton Woodbridge.
				<br/><br/>Call (732) 494-6200 to book your stay or event today.
				<br/><br/>We look forward to seeing you.
				<br/><br/>Hilton Woodbridge
					</div><?	
					}
					else if ($unsubscribed) {
?>					<div class="copy">
					To re-subscribe please visit www.hiltonwoodbridge.com.
					<br/><br/>Thank you.
					<br/><br/>Hilton Woodbridge
					</div>
<?
				  ...
				  <?
					if (!$valid && !$unsubscribed) { //show this form if user did not subscribe to the newsletter
				  ?>
